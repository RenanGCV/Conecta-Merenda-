...
export function OpenAIBadge({ 
  ...
  const sizes = {
    ...
    md: { logo: 20, text: 'text-sm' },
    lg: { logo: 24, text: 'text-base' },
  };

  const variants = {
    light: 'bg-white/90 text-gray-700 border border-gray-200 shadow-sm',
...
// Versão flutuante para canto da tela
export function OpenAIFloatingBadge({ position = 'bottom-right' }: { position?: 'bottom-right' | 'bottom-left' | 'top-right' | 'top-left' }) {
  const positions = {
...
// Versão inline para dentro de cards
export function OpenAISponsorTag({ className = '' }: { className?: string }) {
  return (
